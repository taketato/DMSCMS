...
import { Button, TextField, Paper, Typography, Container, Box, Grid ,Card } from '@mui/material';
...
const HospitalHomePage = () => {
    ...
    const fetchTrackinfo = () => {
      ...
      return fetch(`http://localhost:${PORT_NUM}/admin/hospital/tracking`, {
        method: 'POST',
        headers: {
    ...
    const fetchDemand = () => {
      const token = localStorage.getItem('token');
      const PORT_NUM = 8000;
      const checkingDemand = {
        item_name:itemName_checkingDemand,
        manufacturer:manufacturer_checkingDemand
      };
  
      return fetch(`http://localhost:${PORT_NUM}/admin/hospital/demand`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
        },
        body: JSON.stringify(checkingDemand),
      })
        .then((response) => {
          if (response.ok) {
            return response.json();
    ...
    //=================post new demand=================
    const postDemand = (itemName_new,manufacturer_new,demandAmount_new) => {
      const token = localStorage.getItem('token');
      ...
      fetch(`http://localhost:${PORT_NUM}/admin/hospital/demand/new`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
    ...
    const handlenewDemandSubmit = (e) => {
      e.preventDefault();
      postDemand(itemName_new,manufacturer_new,demandAmount_new);
    };

    //=================update demand=================
    const updateDemand = (itemName_update,manufacturer_update,demandAmount_update) => {
      const token = localStorage.getItem('token');
      const PORT_NUM = 8000;
      const updateDemand = {
        item_id: itemName_update,
        manufacturer: manufacturer_update,
        demand_amount: demandAmount_update,
      };
  
      fetch(`http://localhost:${PORT_NUM}/admin/hospital/demand/update`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
    ...
    const handleupdateDemandSubmit = (e) => {
      e.preventDefault();
      updateDemand(itemName_update,manufacturer_update,demandAmount_update);
    };
          ...
          <Paper elevation={3} style={{ padding: '20px', marginTop: '30px' }}>
            ...
            <Box component="form" onSubmit={handleupdateDemandSubmit} noValidate sx={{ mt: 1 }}>
                ...
                <TextField
                    ...
                    placeholder="Enter Update Demand Amount"
                    margin="normal"
                />
                <Button type="submit" variant="contained" color="primary" sx={{ mt: 3, mb: 2, width: '100%' }}>
                    Update Demand
            ...
            <Box mt={4} mb={2}>
                <Link to="/hospital/inventory" style={{ textDecoration: 'none' }}>
                    <Button variant="outlined" color="secondary">
                        Go to Hospital Inventory
                    </Button>
                </Link>
            </Box>
          </Paper>
